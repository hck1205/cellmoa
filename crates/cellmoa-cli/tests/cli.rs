...
fn csv(scratch: &Scratch, name: &str, text: &str) -> PathBuf {
    let path = scratch.join(name);
    std::fs::write(&path, text).unwrap();
    path
}

const Q3: &str = "name,amount,region\nAlice,1200,East\nBob,1200,West\nCarol,500,East\n";
const Q4: &str = "name,amount,region\nAlice,1200,East\nBob,1350,West\nDave,900,North\n";
...
const WIDE: &str = "Name,Revenue,Quarter,Region,E,F,G,H\n\
...
fn cell_of(path: &Path, reference: &str) -> String {
