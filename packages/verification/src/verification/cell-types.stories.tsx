/**
 * Cell types: the thirteen names that each stand for a renderer, an editor and
 * a validator registered together.
 *
 * A `type` is the shortest configuration in either library and the one most
 * likely to be copied verbatim out of the guide, so the thing to watch across
 * this section is whether a name means the same three functions on both sides.
 * A type that resolves but selects a different editor, or that carries a
 * setting on one side and not the other, is worse than a type that is missing:
 * the configuration reads as understood and the behaviour is not the one asked
 * for. Three of those are in here — `password` and its implied
 * `copyable: false`, `handsontable` and its embedded grid, and the spelling of
 * `multiselect`.
 */
...
/**
 * Autocomplete: a list that suggests rather than constrains.
 */
export const AutocompleteCellType = () => (
  <Compare
    settings={{
      ...
      rowHeaders: true,
      columns: [
        { type: 'autocomplete', source: ['BMW', 'Chrysler', 'Nissan', 'Suzuki', 'Toyota', 'Volvo'] },
...
export const CellType = () => (
  <Compare
    settings={{
      colHeaders: ['text (the default)', 'numeric', 'numeric, renderer: text', 'password'],
      rowHeaders: true,
      columns: [
        {},
  ...
  <Compare
    settings={{
      ...
      columns: [
        ...
        { type: 'intl-date', dateFormat: { month: 'long', day: 'numeric', year: 'numeric' } },
        {
          type: 'intl-date',
  ...
  <Compare
    settings={{
      ...
      columns: [
        ...
        {
          type: 'dropdown',
          source: ['Research and development', 'Sales', 'Facilities management'],
          trimDropdown: false,
        },
      ],
    }}
...
export const MultiSelectCellType = () => (
  <Compare
    ...
    data={[
      ['Los Angeles International', 'Electronics and Gadgets, Medical Supplies'],
  ...
  <Compare
    settings={{
      ...
      columns: [
        ...
        {
          type: 'numeric',
          numericFormat: { style: 'unit', unit: 'kilometer', useGrouping: true },
        },
...
export const PasswordCellType = () => (
  <Compare
    settings={{
      colHeaders: ['User', 'Password', 'Fixed hash length', 'Custom symbol, revealed briefly'],
      rowHeaders: true,
      columns: [
        {},
        { type: 'password' },
    ...
    data={[
      ['Chris Right', 'plainTextPassword', 'plainTextPassword', 'plainTextPassword'],
...
export const SelectCellType = () => (
  <Compare
    settings={{
      ...
      rowHeaders: true,
      colWidths: [80, 120, 90],
      columns: [{}, { type: 'select', selectOptions: ['Kia', 'Nissan', 'Toyota', 'Honda'] }, {}],
    }}
    data={[
      ['2017', 'Honda', '10'],
      ['2018', 'Toyota', '20'],
  ...
  <Compare
    settings={{
      ...
      columns: [
        ...
        { type: 'intl-time', timeFormat: { hour: '2-digit', minute: '2-digit', second: '2-digit' } },
        { type: 'intl-time', timeFormat: { hour: 'numeric', minute: '2-digit', hour12: false } },
      ],
    }}
    data={[
